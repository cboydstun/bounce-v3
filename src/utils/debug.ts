...
export const checkAuthEnvVars = () => {
  ...
  if (!process.env.NEXTAUTH_SECRET && !process.env.JWT_SECRET) missing.push('NEXTAUTH_SECRET or JWT_SECRET');
...
export const authDebug = {
  ...
  error: (message: string, error: any) => {
    debugLog('AUTH_ERROR', message, { 
      message: error?.message, 
      stack: error?.stack,
      name: error?.name
    }, true); // Force log errors even in production
  },
