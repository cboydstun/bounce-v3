...
export function detectDevice(
  ...
  if (
    /android.*mobile|mobile.*android|iphone|ipod|blackberry|iemobile|opera mini/i.test(
      ua,
    )
  ) {
    return "Mobile";
  ...
  // Check for tablets
  if (/ipad|android(?!.*mobile)|tablet|silk|kindle/i.test(ua)) {
    return "Tablet";
  }
...
export function detectBrowser(userAgent: string) {
  ...
  const match = ua.match(
    new RegExp(`${name.toLowerCase()}\\/(\\d+(\\.\\d+)?)`),
  );
