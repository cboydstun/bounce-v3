...
export async function geocodeAddress(address: string): Promise<[number, number]> {
    try {
        ...
        const response = await axios.get(`/api/v1/geocode?address=${encodeURIComponent(address)}`);
        ...
        throw new Error(`Could not find "${address}" in the San Antonio area. Please check the address and try again.`);
    } catch (error) {
        ...
        if (axios.isAxiosError(error)) {
            if (error.response?.status === 404) {
                throw new Error(`Address "${address}" not found in San Antonio area. Please verify the address.`);
