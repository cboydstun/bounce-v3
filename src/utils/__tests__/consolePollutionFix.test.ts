...
describe("Console Pollution Fix Verification", () => {
  ...
  test("should demonstrate clean test output with multiple console calls", () => {
    ...
    expect(console.error).toHaveBeenCalledWith("[Component C] Handling error...");
    expect(console.warn).toHaveBeenCalledWith("[Component D] Warning message...");
    expect(console.info).toHaveBeenCalledWith("[Component E] Info message...");
    expect(console.debug).toHaveBeenCalledWith("[Component F] Debug information...");
  ...
  test("should allow selective debugging when needed", () => {
    ...
    expect(console.log).toHaveBeenCalledWith("This message is mocked and won't appear");
  });
