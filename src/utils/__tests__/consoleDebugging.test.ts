  ...
  describe('debugging component behavior', () => {
    test('should demonstrate debugging a component with console output', () => {
      ...
      if (DEBUG_MODE) {
        ...
        expect(console.log).toHaveBeenCalledWith('[DEBUG] Component: test data');
        expect(console.error).toHaveBeenCalledWith('[ERROR] Component error: test data');
        expect(console.warn).toHaveBeenCalledWith('[WARN] Component warning: test data');
      }
    });
    ...
    test('should demonstrate debugging async operations', async () => {
      ...
      if (DEBUG_MODE) {
        ...
        expect(console.log).toHaveBeenCalledWith('[ASYNC] Starting operation with: test-data');
        expect(console.log).toHaveBeenCalledWith('[ASYNC] Operation completed for: test-data');
      }
    });
  });
  ...
  describe('debugging error scenarios', () => {
    test('should demonstrate debugging error handling', () => {
      ...
      if (DEBUG_MODE) {
        ...
        expect(console.error).toHaveBeenCalledWith('[ERROR] Caught error: Test error message');
        expect(console.warn).toHaveBeenCalledWith('[WARN] Applying fallback behavior');
        expect(console.info).toHaveBeenCalledWith('[INFO] Error handled gracefully');
      }
    });
  });
  ...
  describe('debugging complex data structures', () => {
    test('should demonstrate debugging with complex objects', () => {
      ...
      const processData = (data: typeof complexData) => {
        ...
        console.log('[PROCESS] Output data:', JSON.stringify(processed, null, 2));
        return processed;
      };
      ...
      if (DEBUG_MODE) {
        ...
        expect(console.log).toHaveBeenCalledWith('[PROCESS] Settings:', complexData.settings);
      }
    });
  });
  ...
  describe('performance debugging', () => {
    test('should demonstrate debugging performance issues', () => {
      ...
      const performanceTest = (iterations: number) => {
        ...
        console.log(`[PERF] Completed ${iterations} iterations in ${duration}ms`);
        console.log(`[PERF] Average time per iteration: ${duration/iterations}ms`);
      ...
      if (DEBUG_MODE) {
        ...
        expect(console.log).toHaveBeenCalledWith('[PERF] Starting 500 iterations...');
        expect(console.log).toHaveBeenCalledWith(expect.stringContaining('[PERF] Completed 500 iterations'));
      }
    });
  });
  ...
  describe('debugging instructions', () => {
    test('should provide clear debugging instructions', () => {
      ...
      const instructions = [
        ...
        '',
        '4. Current debug mode:', DEBUG_MODE ? 'ENABLED ✅' : 'DISABLED ❌',
        ''
      ];
