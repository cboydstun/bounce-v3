 ...
 */
export function getReferrerBreakdown(visitors: IVisitor[]): { [key: string]: number } {
 ...
 */
export function getMostVisitedPages(visitors: IVisitor[], limit: number = 5): { 
    page: string; 
    visits: number;
    percentage: number;
}[] {
...
export function getSuspiciousVisitors(visitors: IVisitor[]): IVisitor[] {
    ...
    Object.values(visitorsByFingerprint).forEach(visitorGroup => {
        if (visitorGroup.length > 1) {
            ...
            const uniqueIPs = new Set(visitorGroup.map(v => v.ipAddress).filter(Boolean));
            const uniqueLocations = new Set(
                visitorGroup
                    .filter(v => v.location && v.location.country)
                    .map(v => `${v.location?.country}-${v.location?.region}-${v.location?.city}`)
            );
 ...
 */
export function getBrowserBreakdown(visitors: IVisitor[]): { [key: string]: number } {
 ...
 * Get OS breakdown
 */
export function getOSBreakdown(visitors: IVisitor[]): { [key: string]: number } {
...
export function getEngagementMetrics(visitors: IVisitor[]): {
    ...
    const totalPages = visitors.reduce((sum, visitor) => sum + visitor.visitedPages.length, 0);
