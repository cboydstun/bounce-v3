...
describe("Single Product API", () => {
    ...
    describe("GET /api/v1/products/[slug]", () => {
        it("should return 404 if product not found", async () => {
            ...
            const response = await GET(req, { params: Promise.resolve({ slug: "non-existent-product" }) });
        ...
        it("should return product if found", async () => {
            const req = new NextRequest(`http://localhost:3000/api/v1/products/${productSlug}`);

            const response = await GET(req, { params: Promise.resolve({ slug: productSlug }) });
    ...
    describe("PUT /api/v1/products/[slug]", () => {
        it("should return 403 if user is not admin", async () => {
            ...
            (withAuth as jest.Mock).mockImplementationOnce((req, handler) => {
                return handler({
                    ...
                    json: () => Promise.resolve({ price: { base: 120, currency: "USD" } }),
                });
            });
            ...
            const response = await PUT(req, { params: Promise.resolve({ slug: productSlug }) });
        ...
        it("should return 404 if product not found", async () => {
            ...
            const response = await PUT(req, { params: Promise.resolve({ slug: "non-existent-product" }) });
            expect(response.status).toBe(404);
    ...
    describe("DELETE /api/v1/products/[slug]", () => {
        it("should return 403 if user is not admin", async () => {
            ...
            const response = await DELETE(req, { params: Promise.resolve({ slug: productSlug }) });
            expect(response.status).toBe(403);
        ...
        it("should return 404 if product not found", async () => {
            ...
            const response = await DELETE(req, { params: Promise.resolve({ slug: "non-existent-product" }) });
            expect(response.status).toBe(404);
