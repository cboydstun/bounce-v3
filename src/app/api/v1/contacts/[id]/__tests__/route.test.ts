...
describe("Single Contact API", () => {
    ...
    describe("GET /api/v1/contacts/[id]", () => {
        it("should return 401 if user is not authenticated", async () => {
            ...
            const response = await GET(req, { params: Promise.resolve({ id: contactId }) });
            expect(response.status).toBe(401);

            const data = await response.json();
            // Accept either error message
            expect(["Unauthorized - No token provided", "Unauthorized - Invalid token"]).toContain(data.error);
        });
    ...
    describe("PUT /api/v1/contacts/[id]", () => {
        it("should return 401 if user is not authenticated", async () => {
            ...
            const response = await PUT(req, { params: Promise.resolve({ id: contactId }) });
            expect(response.status).toBe(401);
    ...
    describe("DELETE /api/v1/contacts/[id]", () => {
        it("should return 403 if user is not admin", async () => {
            ...
            const response = await DELETE(req, { params: Promise.resolve({ id: contactId }) });
