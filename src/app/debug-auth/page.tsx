...
export default function DebugAuthPage() {
  ...
  // Log session status
  useEffect(() => {
    authDebug.log("Session status in debug page", {
      status,
      hasSession: !!session,
    });
    ...
    try {
      ...
      if (vars.missing.length > 0) {
        setError(
          `Missing required environment variables: ${vars.missing.join(", ")}`,
        );
      }
    } catch (err) {
  ...
  const checkSessionManually = async () => {
    try {
      ...
      setError(
        `getSession() error: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  };
  ...
  const testLogin = async () => {
    try {
      ...
      setError(
        `Login test error: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  };
  ...
  const testLogout = async () => {
    try {
      ...
      setError(
        `Logout test error: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  };
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
          ...
          <div className="text-sm">
            ...
            {session && session.user && (
              <>
                ...
                <p>
                  <strong>Expires:</strong> {session.expires || "Not set"}
                </p>
              </>
            )}
          </div>
            ...
            {sessionChecked && (
              <div className="mt-2 text-sm">
                <p>
                  ...
                  {getSessionResult ? "Session exists" : "No session"}
                </p>
                {getSessionResult && getSessionResult.user && (
                  <>
                    <p>
                      <strong>User ID:</strong>{" "}
                      {getSessionResult.user.id || "Not set"}
                    </p>
                    <p>
                      <strong>User Email:</strong>{" "}
                      {getSessionResult.user.email || "Not set"}
                    </p>
                  </>
                )}
              </div>
            ...
            {envVars && (
              <>
                <p>
                  ...
                  {envVars.vars.NODE_ENV || "Not set"}
                </p>
                <p>
                  <strong>NEXTAUTH_URL:</strong>{" "}
                  {envVars.vars.NEXTAUTH_URL_SET ? "Set" : "Not set"}
                </p>
                <p>
                  <strong>NEXTAUTH_SECRET:</strong>{" "}
                  {envVars.vars.NEXTAUTH_SECRET_SET ? "Set" : "Not set"}
                </p>
                <p>
                  <strong>JWT_SECRET:</strong>{" "}
                  {envVars.vars.JWT_SECRET_SET ? "Set" : "Not set"}
                </p>
                <p>
                  <strong>MONGODB_URI:</strong>{" "}
                  {envVars.vars.MONGODB_URI_SET ? "Set" : "Not set"}
                </p>
      ...
      <div className="mt-6 text-sm text-gray-600">
        <p>
          This page is for debugging authentication issues. It displays
          sensitive information and should not be accessible in production.
        </p>
      </div>
    </div>
  );
