...
export default function TaskEditModal({
  ...
  const [formData, setFormData] = useState<
    Partial<
      Omit<TaskFormData, "paymentAmount" | "scheduledDateTime"> & {
  ...
  useEffect(() => {
    if (task && isOpen) {
      setFormData({
        ...
        scheduledDateTime:
          typeof task.scheduledDateTime === "string"
            ? task.scheduledDateTime
            : new Date(task.scheduledDateTime).toISOString().slice(0, 16),
        priority: task.priority,
        status: task.status,
        assignedContractors: task.assignedContractors || [],
        paymentAmount: task.paymentAmount?.toString() || "",
      });
      setSelectedOrder(null); // Will be loaded by OrderSelector
      setErrors({});
    }
  }, [task, isOpen]);
        ...
        <div className="mt-3">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-lg font-medium text-gray-900">
              Edit Task: {task.title || `Task ${task._id.slice(-6)}`}
            </h3>
            <button
              onClick={handleClose}
              disabled={loading}
