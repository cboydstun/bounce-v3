...
const ReferrerSourceChart: React.FC<ReferrerSourceChartProps> = ({ referrerData }) => {
    ...
    const directPercentage = total > 0 ? ((directTraffic / total) * 100).toFixed(1) : "0";
    const referralPercentage = total > 0 ? ((referralTraffic / total) * 100).toFixed(1) : "0";
    ...
    return (
        <div className="bg-white p-4 rounded-lg shadow">
            ...
            <div className="mt-4 text-sm text-gray-600">
                ...
                {topReferrer === 'Direct' ? (
                    <p>Most visitors come directly to your site. Consider investing in more referral channels to expand your reach.</p>
                ) : topReferrer === 'Google' ? (
                    <p>Google is your top traffic source. Ensure your SEO strategy is optimized for rental-related keywords.</p>
                ) : topReferrer.includes('Facebook') || topReferrer.includes('Instagram') ? (
                    <p>Social media is driving significant traffic. Consider increasing your presence on {topReferrer} with targeted ads for your rental business.</p>
                ) : (
                    <p>Your top traffic source is {topReferrer}. Focus marketing efforts on this channel to maximize ROI.</p>
                )}
            </div>
        </div>
    );
