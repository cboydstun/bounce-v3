...
export default function FormEngagementAnalytics({
  visitors,
}: FormEngagementAnalyticsProps) {
  // State for form type filter
  const [formType, setFormType] = React.useState<"all" | "contact" | "coupon">(
    "all",
  );

  // Extract form interactions from visitors
  const allFormInteractions = visitors.flatMap(
    (visitor) =>
  ...
  const formStarts = formInteractions.filter(
    (i) => i.type === "form_start",
  ).length;
  const formSubmits = formInteractions.filter(
    (i) => i.type === "form_submit",
  ).length;
  const formCompletions = formInteractions.filter(
    (i) => i.type === "contact_form",
  ).length;
  const successPageViews = formInteractions.filter(
    (i) => i.type === "success_page_view",
  ).length;
  const ctaClicks = formInteractions.filter(
    (i) => i.type === "success_page_cta_click",
  ).length;

  // Calculate conversion rates
  const startToSubmitRate =
    formStarts > 0 ? ((formSubmits / formStarts) * 100).toFixed(1) : "0";
  const submitToCompletionRate =
    formSubmits > 0 ? ((formCompletions / formSubmits) * 100).toFixed(1) : "0";
  const completionToSuccessRate =
    formCompletions > 0
      ? ((successPageViews / formCompletions) * 100).toFixed(1)
      : "0";
  const successToCTARate =
    successPageViews > 0
      ? ((ctaClicks / successPageViews) * 100).toFixed(1)
      : "0";

  // Field completion analysis
  const fieldInteractions = formInteractions.filter(
    (i) => i.type === "form_field_complete",
  );
  ...
  const extrasLabels = Object.keys(extrasDistribution).map(
    (k) => `${k} extras`,
  );
  ...
  const contactFormCount = allFormInteractions.filter(
    (i) =>
      (i.element || "").includes("contact-form") && i.type === "form_start",
  ).length;

  const couponFormCount = allFormInteractions.filter(
    (i) => (i.element || "").includes("coupon-form") && i.type === "form_start",
  ).length;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-green-50 p-4 rounded-lg">
            ...
            <p className="text-xs text-gray-500">
              Forms submitted to completed
            </p>
          </div>

          <div className="bg-purple-50 p-4 rounded-lg">
            <p className="text-sm text-purple-600 font-medium">
              Success Page View Rate
            </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          <div className="mt-4 text-sm text-gray-600">
            <p>
              This chart shows which form fields users are most likely to
              complete, helping identify potential friction points in your form.
            </p>
          </div>
        </div>

        {/* Extras Selection Distribution - Only show for contact form */}
        {(formType === "all" || formType === "contact") &&
          extrasSelections.length > 0 && (
            <div className="bg-white p-6 rounded-lg shadow">
              <h3 className="text-lg font-semibold mb-4">
                Extras Selection Distribution
              </h3>
              <div className="h-64">
                <Pie
              ...
              <div className="mt-4 text-sm text-gray-600">
                <p>
                  This chart shows how many extras users typically select,
                  helping you understand which add-ons are most popular.
                </p>
              </div>
            </div>
          )}
