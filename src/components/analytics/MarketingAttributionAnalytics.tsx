...
const MarketingAttributionAnalytics: React.FC<
  MarketingAttributionAnalyticsProps
> = ({ visitors }) => {
  // Count visitors with UTM parameters
  const visitorsWithUtm = visitors.filter(
    (v) => v.utmSource || v.utmMedium || v.utmCampaign,
  );

  const percentageWithUtm =
    visitors.length > 0 ? (visitorsWithUtm.length / visitors.length) * 100 : 0;
  ...
  const sortedUtmSources = Object.entries(utmSourceCounts).sort(
    (a, b) => b[1] - a[1],
  );
  ...
  const sortedUtmMediums = Object.entries(utmMediumCounts).sort(
    (a, b) => b[1] - a[1],
  );
  ...
  const sortedUtmCampaigns = Object.entries(utmCampaignCounts).sort(
    (a, b) => b[1] - a[1],
  );

  // Calculate conversion rates by source
  const conversionsBySource: Record<
    string,
    { visitors: number; conversions: number; rate: number }
  > = {};
  ...
  visitorsWithUtm.forEach((visitor) => {
    ...
    if (
      ...
      (visitor.conversionEvents &&
        visitor.conversionEvents.some((e) => e.type === "booking_completed"))
    ) {
  ...
  Object.keys(conversionsBySource).forEach((source) => {
    ...
    conversionsBySource[source].rate =
      visitors > 0 ? (conversions / visitors) * 100 : 0;
  });

  // Sort sources by conversion rate
  const sortedByConversionRate = Object.entries(conversionsBySource).sort(
    (a, b) => b[1].rate - a[1].rate,
  );

  // Calculate engagement scores by source
  const engagementBySource: Record<
    string,
  ...
  const sortedByEngagement = Object.entries(engagementBySource).sort(
  ...
  const landingPageEffectiveness: Record<
    string,
    { visitors: number; conversions: number; rate: number }
  > = {};
    ...
    <div className="bg-white p-4 rounded-lg shadow">
      ...
      <div className="mb-6">
        <div className="flex justify-between items-center mb-2">
          ...
          <span className="text-sm text-gray-500">
            {visitorsWithUtm.length} of {visitors.length} visitors (
            {percentageWithUtm.toFixed(1)}%)
          </span>
        </div>
                ...
                <div key={index} className="flex items-center">
                  <div className="w-full bg-gray-200 rounded-full h-4 mr-2">
                    <div
                      ...
                      style={{
                        width: `${(count / visitorsWithUtm.length) * 100}%`,
                      }}
                    ></div>
                  </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {sortedUtmCampaigns
                  ...
                  .map(([campaign, count], index) => {
                    ...
                    const campaignVisitors = visitorsWithUtm.filter(
                      (v) => v.utmCampaign === campaign,
                    );
                    ...
                    const topMedium =
                      Object.entries(mediumCounts).sort(
                        (a, b) => b[1] - a[1],
                      )[0]?.[0] || "Unknown";
      ...
      <div className="mt-6 text-sm text-gray-600">
        ...
        <ul className="list-disc list-inside mt-2 space-y-2">
          {percentageWithUtm < 50 && (
            <li>
              Only {percentageWithUtm.toFixed(1)}% of your visitors have UTM
              parameters. Add UTM parameters to all your marketing campaigns for
              better attribution.
            </li>
          )}

          {sortedByConversionRate.length > 0 &&
            sortedByConversionRate[0][1].rate > 0 && (
              <li>
                Your highest converting traffic source is{" "}
                {sortedByConversionRate[0][0]} with a{" "}
                {sortedByConversionRate[0][1].rate.toFixed(1)}% conversion rate.
                Consider increasing your marketing budget for this channel.
              </li>
            )}

          {sortedByConversionRate.length > 1 &&
            sortedByConversionRate[sortedByConversionRate.length - 1][1]
              .visitors > 10 &&
            sortedByConversionRate[sortedByConversionRate.length - 1][1].rate <
              1 && (
              <li>
                Your{" "}
                {sortedByConversionRate[sortedByConversionRate.length - 1][0]}{" "}
                traffic source has a low conversion rate (
                {sortedByConversionRate[
                  sortedByConversionRate.length - 1
                ][1].rate.toFixed(1)}
                %). Consider optimizing or reducing spend on this channel.
              </li>
            )}

          {sortedLandingPages.length > 0 &&
            sortedLandingPages[0][1].rate > 5 && (
              <li>
                Your best performing landing page is "
                {formatLandingPage(sortedLandingPages[0][0])}" with a{" "}
                {sortedLandingPages[0][1].rate.toFixed(1)}% conversion rate.
                Direct more traffic to this page.
              </li>
            )}

          {sortedUtmMediums.length > 0 &&
            sortedUtmMediums[0][0] !== "Unknown" && (
              <li>
                Your top traffic medium is {sortedUtmMediums[0][0]}. Ensure
                you're optimizing your content for this channel.
              </li>
            )}

          {sortedUtmCampaigns.length > 0 &&
            sortedUtmCampaigns[0][0] !== "Unknown" && (
              <li>
                Your most active campaign is "{sortedUtmCampaigns[0][0]}" which
                has brought in {sortedUtmCampaigns[0][1]} visitors.
              </li>
            )}
        </ul>
      </div>
