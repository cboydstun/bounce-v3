...
const PopularPagesChart: React.FC<PopularPagesChartProps> = ({ pagesData }) => {
            ...
            tooltip: {
                callbacks: {
                    label: function(context: any) {
                        ...
                        const percentage = pagesData[context.dataIndex].percentage.toFixed(1);
    ...
    const productPages = pagesData.filter(page => page.page.includes('/products/'));
    const hasProductPages = productPages.length > 0;
    
    // Get the most popular product if any
    const mostPopularProduct = productPages.length > 0 
        ? productPages.reduce((prev, current) => (prev.visits > current.visits) ? prev : current)
        : null;
        ...
        <div className="bg-white p-4 rounded-lg shadow">
            ...
            <div className="mt-4 text-sm text-gray-600">
                ...
                {hasProductPages ? (
                    <div>
                        <p>
                            Your most popular rental product is <span className="font-semibold">
                                {formatPageUrl(mostPopularProduct!.page).replace('Product: ', '')}
                            </span> with {mostPopularProduct!.visits} views.
                        </p>
                        <p className="mt-2">
                            Consider featuring this product prominently on your homepage and ensuring 
                            you have sufficient inventory available.
                        </p>
                    </div>
                ) : (
                    <p>
                        Your most visited page is <span className="font-semibold">{labels[0]}</span>. 
                        Consider optimizing this page for conversions and ensuring it showcases your 
