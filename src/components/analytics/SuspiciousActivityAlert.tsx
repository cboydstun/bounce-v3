...
const SuspiciousActivityAlert: React.FC<SuspiciousActivityAlertProps> = ({ visitors }) => {
    ...
    const groupByFingerprint = () => {
        ...
        return Object.entries(groups)
            ...
            .map(([fingerprint, group]) => ({
                fingerprint,
                visitors: group,
                locations: [...new Set(group.map(v => 
                    v.location ? `${v.location.city || ''}, ${v.location.country || ''}` : 'Unknown'
                ))],
                ips: [...new Set(group.map(v => v.ipAddress).filter(Boolean))],
                lastActivity: new Date(Math.max(...group.map(v => new Date(v.lastVisit).getTime())))
            }))
    ...
    return (
        <div className={`bg-white p-4 rounded-lg shadow ${hasSuspiciousActivity ? 'border-l-4 border-red-500' : ''}`}>
            <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-semibold">Security Alerts</h3>
                {hasSuspiciousActivity && (
                    <span className="px-2 py-1 text-xs font-medium rounded-full bg-red-100 text-red-800">
                        {suspiciousGroups.length} Alert{suspiciousGroups.length !== 1 ? 's' : ''}
                    </span>
                )}
            </div>
            ...
            {!hasSuspiciousActivity ? (
                <div className="bg-green-50 p-4 rounded-lg">
                    ...
                    <p className="text-sm text-green-600 mt-2">
                        All visitor fingerprints are associated with consistent location and IP data.
                    </p>
                </div>
            ) : (
                <div>
                    <div className="bg-red-50 p-4 rounded-lg mb-4">
                        <div className="flex items-start">
                            ...
                            <div>
                                <p className="text-red-700 font-medium">Suspicious activity detected</p>
                                <p className="text-sm text-red-600 mt-1">
                                    {suspiciousGroups.length} fingerprint{suspiciousGroups.length !== 1 ? 's' : ''} associated with multiple locations or IP addresses.
                                    This may indicate fake leads, competitor research, or automated bots.
                                </p>
                            </div>
                        </div>
                        <button 
                            ...
                            {suspiciousGroups.map((group, index) => (
                                <div key={index} className="border border-gray-200 rounded-lg overflow-hidden">
                                    <div className="bg-gray-50 p-3 border-b border-gray-200">
                                        <p className="font-medium">Fingerprint: {group.fingerprint.substring(0, 8)}...</p>
            ...
            <div className="mt-6 text-sm text-gray-600">
                ...
                <p className="mt-2">
                    Consider implementing CAPTCHA for contact forms if suspicious activity is frequent.
                </p>
            </div>
        </div>
    );
