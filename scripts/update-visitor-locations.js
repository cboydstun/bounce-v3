...
async function updateVisitorLocations() {
    try {
        ...
        for (let i = 0; i < visitors.length; i += batchSize) {
            ...
            console.log(`Processing batch ${Math.floor(i / batchSize) + 1} of ${Math.ceil(visitors.length / batchSize)}`);
            ...
            const promises = batch.map(async (visitor) => {
                try {
                    ...
                    if (!ipAddress || ipAddress === 'unknown') {
                        console.log(`Skipping visitor ${visitor._id} with invalid IP: ${ipAddress}`);
                        return;
                    }
                    
                    console.log(`Getting location for visitor ${visitor._id} with IP: ${ipAddress}`);
                    const locationData = await getLocationFromIp(ipAddress);
                    
                    if (locationData) {
                        console.log(`Updating visitor ${visitor._id} with location: ${locationData.city}, ${locationData.region}, ${locationData.country}`);
                        visitor.location = locationData;
                        await visitor.save();
                    } else {
                        console.log(`No location data found for visitor ${visitor._id} with IP: ${ipAddress}`);
                    }
                } catch (error) {
