...
async function checkEnvironmentVariables() {
  ...
  if (!process.env.NEXTAUTH_URL) {
    ...
    if (!isValidUrl(config.url)) {
      ...
      config.url = await question(
        "Please enter your site URL (e.g., https://example.com): ",
      );
...
async function testCookieSettings() {
  ...
  try {
    ...
    if (!response.ok) {
      log.error(
        `Failed to access ${config.url}: ${response.status} ${response.statusText}`,
      );
      return;
    }
...
async function testAuthFlow() {
  ...
  try {
    ...
    const loginResponse = await fetch(
      `${config.url}/api/auth/callback/credentials`,
      {
        method: "POST",
        headers: {
...
async function main() {
  ...
  try {
    ...
    log.info(
      "Check the logs above for any issues with your NextAuth.js configuration",
    );
  } catch (error) {
