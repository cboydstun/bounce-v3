...
async function testNextAuth() {
  ...
  try {
    ...
    log(`CSRF endpoint status: ${csrfResponse.statusCode}`, 
      csrfResponse.statusCode === 200 ? colors.green : colors.yellow);
    
    if (csrfResponse.statusCode === 200 && csrfResponse.data.csrfToken) {
      log(`CSRF token obtained: ${csrfResponse.data.csrfToken.substring(0, 10)}...`, colors.green);
    } else {
    ...
    if (loginResponse.statusCode === 200) {
      ...
      log(`Session endpoint status: ${sessionResponse.statusCode}`, 
        sessionResponse.statusCode === 200 ? colors.green : colors.red);
      
      if (sessionResponse.statusCode === 200) {
        if (sessionResponse.data.user) {
          log('Session contains user data:', colors.green);
          log(`  User ID: ${sessionResponse.data.user.id || 'Not set'}`, colors.green);
          log(`  User Email: ${sessionResponse.data.user.email || 'Not set'}`, colors.green);
        } else {
          log('Session endpoint returned success but no user data found', colors.yellow);
        }
      } else {
      ...
      log(`Signout response status: ${signoutResponse.statusCode}`, 
      ...
      if (signoutResponse.statusCode === 200) {
        ...
        if (postSignoutSessionResponse.data.user) {
          log('Warning: Session still contains user data after signout', colors.yellow);
        } else {
