...
describe('Task Management Integration Tests', () => {
  ...
  describe('GET /api/tasks/available', () => {
    ...
    beforeEach(async () => {
      ...
      authHeaders = TestHelpers.getAuthHeaders(testContractor._id, testContractor);
      ...
      await TestHelpers.createTestTask({
        ...
        location: {
          type: 'Point',
          coordinates: [-98.5200, 29.4500], // Further location (about 3km away)
        },
      });
  ...
  describe('POST /api/tasks/:id/claim', () => {
    ...
    it('should reject claiming task without required skills', async () => {
      ...
      const wrongAuthHeaders = TestHelpers.getAuthHeaders(contractorWithoutSkills._id, contractorWithoutSkills);
  ...
  describe('PUT /api/tasks/:id/status', () => {
    ...
    it('should reject updating task not assigned to contractor', async () => {
      ...
      const otherAuthHeaders = TestHelpers.getAuthHeaders(otherContractor._id, otherContractor);
  ...
  describe('GET /api/tasks/:id', () => {
    ...
    it('should reject access to task assigned to other contractor', async () => {
      ...
      expect(response.body).toHaveProperty('error');
      expect(response.body.error).toContain('You do not have access to this task');
    });
  ...
  describe('Task Workflow Integration', () => {
    ...
    it('should complete full task workflow: claim -> start -> complete', async () => {
      ...
      expect(completeResponse.body.task.completionNotes).toBe('Task completed successfully');
