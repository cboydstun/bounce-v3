...
export class TestHelpers {
  static async createTestContractor(
  ...
  static async createTestTask(overrides: Partial<any> = {}): Promise<any> {
    ...
    const task = new (Task as any)(taskData) as {
      _id: { toString: () => string };
    } & typeof taskData;
    await task.save();
  ...
  static generateJWTToken(
    contractorId: string,
    contractorData?: Partial<TestContractor>,
  ): string {
    const payload = {
      contractorId,
  ...
  static async createTestQuickBooksToken(
    contractorId: string,
    overrides: any = {},
  ): Promise<any> {
