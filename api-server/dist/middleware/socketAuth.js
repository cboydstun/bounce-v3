...
export const socketAuthMiddleware = async (socket, next) => {
    try {
        ...
        if (!contractor) {
            logger.warn(`Socket connection rejected - contractor not found: ${payload.contractorId}`);
            return next(new Error("Contractor not found"));
        }
        if (!contractor.isActive) {
            logger.warn(`Socket connection rejected - contractor inactive: ${payload.contractorId}`);
            return next(new Error("Contractor account is inactive"));
        }
        if (!contractor.isVerified) {
            logger.warn(`Socket connection rejected - contractor not verified: ${payload.contractorId}`);
        ...
        logger.info(`Socket authenticated successfully: ${socket.id} for contractor: ${contractor._id}`);
        next();
    }
    catch (error) {
...
export class SocketRateLimiter {
    static eventCounts = new Map();
    static MAX_EVENTS_PER_MINUTE = 50;
    static WINDOW_MS = 60 * 1000; // 1 minute
    static checkRateLimit(socketId) {
        const now = Date.now();
        const key = socketId;
