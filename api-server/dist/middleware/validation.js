...
const skillsSchema = Joi.array()
    .items(Joi.string().trim().min(1).max(50).messages({
...
export const w9FormSchema = Joi.object({
    ...
    taxClassification: Joi.string()
        .valid("individual", "c-corp", "s-corp", "partnership", "trust", "llc", "other")
        .required()
        .messages({
        "any.only": "Tax classification must be one of: individual, c-corp, s-corp, partnership, trust, llc, other",
    ...
    taxClassificationOther: Joi.string()
        ...
        .messages({
        "string.max": "Tax classification description must not exceed 100 characters",
        "any.required": 'Tax classification description is required when "other" is selected',
    }),
    taxId: taxIdSchema,
