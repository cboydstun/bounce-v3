...
export class QuickBooksController {
    ...
    async getW9Status(req, res) {
        try {
            const contractorId = new mongoose.Types.ObjectId((req.user || req.contractor).id);
    ...
    async downloadW9PDF(req, res) {
      ...
      `);
            ...
            res.setHeader("Content-Disposition", `attachment; filename="w9-form-${w9Form.businessName.replace(/[^a-zA-Z0-9]/g, "-")}.pdf"`);
    ...
    async getQuickBooksStatus(req, res) {
        try {
            ...
            const connectionStatus = await quickbooksService.instance.getConnectionStatus(contractorId);
            // Get W-9 status
            const w9Form = await W9Form.findByContractor(contractorId);
            const syncStatus = {
                ...
                readyForSync: connectionStatus.connected && w9Form?.status === "approved",
    ...
    async syncContractor(req, res) {
        try {
            ...
            const vendorId = await quickbooksService.instance.createVendor(contractorId, vendorData);
            res.json({
                success: true,
                data: {
                    vendorId,
    ...
    async handleOAuthCallback(req, res) {
        try {
            ...
            const tokens = await quickbooksService.instance.exchangeCodeForTokens(code, state, realmId);
            // Redirect to mobile app with success
            res.redirect(`${process.env.MOBILE_APP_URL}/quickbooks/callback?success=true`);
        }
        catch (error) {
            logger.error("QuickBooks OAuth callback error:", error);
            res.redirect(`${process.env.MOBILE_APP_URL}/quickbooks/callback?success=false&error=${encodeURIComponent("OAuth callback failed")}`);
        }
    }
}
