...
router.get("/available", authenticateToken, requireVerified, validate(availableTasksSchema, "query"), TaskController.getAvailableTasks);
// GET /api/tasks/my-tasks - Get contractor's assigned tasks
router.get("/my-tasks", authenticateToken, requireVerified, validate(myTasksSchema, "query"), TaskController.getMyTasks);
// GET /api/tasks/:id - Get task details by ID
router.get("/:id", authenticateToken, requireVerified, validate(taskIdSchema, "params"), TaskController.getTaskById);
// POST /api/tasks/:id/claim - Claim an available task
router.post("/:id/claim", authenticateToken, requireVerified, validate(taskIdSchema, "params"), TaskController.claimTask);
// PUT /api/tasks/:id/status - Update task status
router.put("/:id/status", authenticateToken, requireVerified, validate(taskIdSchema, "params"), validate(updateStatusSchema, "body"), TaskController.updateTaskStatus);
// POST /api/tasks/:id/complete - Complete task with photos and notes
router.post("/:id/complete", authenticateToken, requireVerified, validate(taskIdSchema, "params"), validate(completeTaskSchema, "body"), TaskController.completeTask);
