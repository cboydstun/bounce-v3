...
export class PDFGeneratorService {
    ...
    async generateW9PDF(w9Data) {
        try {
            ...
            page.drawText("3. Check appropriate box for federal tax classification:", {
                x: 50,
                y: yPosition,
                size: fontSize,
                color: rgb(0, 0, 0),
            ...
            if (w9Data.exemptPayeeCodes && w9Data.exemptPayeeCodes.length > 0) {
                page.drawText(`Exempt payee code: ${w9Data.exemptPayeeCodes.join(", ")}`, {
                    x: 70,
                    y: yPosition - 20,
                    size: fontSize - 1,
                    color: rgb(0, 0, 0),
            ...
            yPosition -= 30;
            page.drawText("Enter your TIN in the appropriate box. The TIN provided must match the name given on line 1.", {
                x: 50,
                y: yPosition,
                size: fontSize - 1,
                color: rgb(0, 0, 0),
     ...
     */
    async savePDFToFile(pdfBuffer, filename, directory) {
        try {
            const saveDirectory = directory || path.join(process.cwd(), "temp", "pdfs");
     ...
     */
    generateW9Filename(contractorId, timestamp) {
        const date = timestamp || new Date();
        const dateString = date.toISOString().split("T")[0]; // YYYY-MM-DD
        const timeString = date.toTimeString().split(" ")[0]?.replace(/:/g, "-") || "00-00-00"; // HH-MM-SS
...
export const generateW9PDF = (w9Data) => {
    return pdfGeneratorService.generateW9PDF(w9Data);
};
export const saveW9PDF = (pdfBuffer, contractorId, directory) => {
