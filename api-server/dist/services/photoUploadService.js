...
export class PhotoUploadService {
    ...
    static initialize() {
        ...
        if (!cloudName || !apiKey || !apiSecret) {
            logger.warn("Cloudinary configuration missing. Photo upload will be disabled.");
            return;
        }
        cloudinary.config({
     ...
     */
    static async uploadPhoto(photoData, options = {}) {
        try {
            if (!this.isConfigured) {
                this.initialize();
                if (!this.isConfigured) {
     ...
     * @param options Upload options
     */
    static async uploadMultiplePhotos(photosData, options = {}) {
        const uploadPromises = photosData.map((photoData, index) => this.uploadPhoto(photoData, {
            ...options,
    ...
    static generateSignedUploadUrl(options = {}) {
        try {
            ...
            const uploadOptions = {
                timestamp,
                folder: options.folder || "task-completions",
                transformation: options.transformation || "w_1200,h_1200,c_limit,q_auto:good",
            ...
            const signature = cloudinary.utils.api_sign_request(uploadOptions, apiSecret);
            const cloudName = process.env.CLOUDINARY_CLOUD_NAME;
            const apiKey = process.env.CLOUDINARY_API_KEY;
            if (!cloudName || !apiKey) {
...
//# sourceMappingURL=photoUploadService.js.map