...
export class TaskService {
    ...
    static async getAvailableTasks(contractorId, filters) {
        try {
            logger.info(`[DEBUG] getAvailableTasks called with contractorId: ${contractorId}`);
            logger.info(`[DEBUG] Filters:`, filters);
            const { lat, lng, radius = 50, // Default 50km radius
            skills = [], page = 1, limit = 20, } = filters;
            ...
            if (lat && lng) {
                ...
                const tasks = await Task.findAvailableNearLocation(lat, lng, radiusInMeters, skills.length > 0 ? skills : contractorSkills, contractorId);
                // Apply pagination
                ...
                for (const taskType of taskTypes) {
                    for (const skill of skills) {
                        if (skill && typeof skill === "string") {
                            ...
                            if (taskTypeLower.includes(skillLower) ||
            ...
            logger.info(`[DEBUG] Task details:`, tasks.map((t) => ({
                id: t._id,
                status: t.status,
                type: t.type,
        ...
        try {
            ...
            if (contractor.skills && contractor.skills.length > 0) {
                ...
                const hasMatchingSkill = contractor.skills.some((skill) => {
                    ...
                    return (skillLower.includes(taskType) ||
                        taskType.includes(skillLower) ||
                        // Allow "delivery" contractors to do "setup" and vice versa
                        (taskType === "setup" && (skillLower.includes("delivery") || skillLower.includes("install"))) ||
                        (taskType === "delivery" && (skillLower.includes("setup") || skillLower.includes("transport"))) ||
                ...
                if (!hasMatchingSkill) {
                    logger.warn(`Contractor ${contractorId} with skills [${contractor.skills.join(', ')}] attempted to claim ${taskType} task ${taskId}`);
                    return {
                        success: false,
                ...
                logger.info(`Contractor ${contractorId} has no skills defined, allowing task claim`);
            }
    ...
    static async updateTaskStatus(taskId, contractorId, newStatus) {
        try {
            ...
            logger.info(`Task ${taskId} status updated to ${newStatus} by contractor ${contractorId}`);
            // Broadcast real-time event for status update
            if (RealtimeService.isInitialized()) {
                await RealtimeService.broadcastTaskStatusUpdate(updatedTask, previousStatus, contractorId);
            }
            return {
                success: true,
                task: updatedTask,
    ...
    static async getTaskById(taskId, contractorId) {
        try {
            ...
            const hasAccess = task.status === "Pending" || // Available tasks
    ...
    static async getAllTasksDebug() {
        try {
            ...
            logger.info(`[DEBUG] Tasks found:`, tasks.map((t) => ({
                id: t._id,
                status: t.status,
                type: t.type,
                assignedContractors: t.assignedContractors,
