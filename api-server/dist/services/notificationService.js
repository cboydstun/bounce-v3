...
export class NotificationService {
        ...
        try {
            const notifications = contractorIds.map((contractorId) => ({
                ...
                expiresAt: notificationData.expiresInHours
                    ? new Date(Date.now() + notificationData.expiresInHours * 60 * 60 * 1000)
                    : null,
            }));
    ...
    static async getNotifications(filters) {
        try {
            const { contractorId, type, priority, read, delivered, page = 1, limit = 20, } = filters;
            const query = {};
            if (contractorId) {
    ...
    static async markAsRead(notificationId, contractorId) {
        try {
            ...
            if (result.modifiedCount > 0) {
                logger.info(`Notification marked as read: ${notificationId} by contractor: ${contractorId}`);
                return true;
            }
            return false;
        }
    ...
    static async markMultipleAsRead(notificationIds, contractorId) {
        try {
            ...
            logger.info(`${result.modifiedCount} notifications marked as read by contractor: ${contractorId}`);
    ...
    static async getUndeliveredNotifications(contractorId) {
        try {
            const notifications = await Notification.find({
                ...
                .exec();
            logger.info(`Retrieved ${notifications.length} undelivered notifications for contractor: ${contractorId}`);
    ...
    static async deleteNotification(notificationId, contractorId) {
        try {
            ...
            if (result.deletedCount > 0) {
                logger.info(`Notification deleted: ${notificationId} by contractor: ${contractorId}`);
                return true;
            }
            return false;
        }
