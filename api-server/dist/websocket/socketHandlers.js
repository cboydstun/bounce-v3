import { SocketRateLimiter, } from "../middleware/socketAuth.js";
...
export class SocketHandlers {
    ...
    async handleConnection(socket) {
        ...
        try {
            ...
            logger.error(`Error setting up socket for contractor ${contractorId}:`, error);
    ...
    setupSocketEventHandlers(socket) {
        ...
        socket.on("contractor:location-update", async (data) => {
        ...
        socket.on("task:subscribe", async (filters) => {
            if (!this.checkRateLimit(socket))
                return;
            try {
                logger.info(`Contractor ${contractorId} subscribed to task updates with filters:`, filters);
                ...
                logger.error(`Error subscribing to tasks for contractor ${contractorId}:`, error);
                socket.emit("error", {
        ...
        socket.on("notification:ack", (data) => {
            if (!this.checkRateLimit(socket))
                return;
            logger.info(`Notification acknowledged by contractor ${contractorId}:`, data.notificationId);
        ...
        socket.on("disconnect", async (reason) => {
            logger.info(`Contractor ${contractorId} disconnected: ${reason} (Socket: ${socket.id})`);
            try {
                await this.roomManager.leaveAllRooms(socket);
            }
            catch (error) {
                logger.error(`Error cleaning up rooms for contractor ${contractorId}:`, error);
            }
        });
     ...
     */
    async broadcastTaskEvent(eventType, taskData, options) {
        try {
            const { excludeContractor, location, skills, targetContractor } = options || {};
            // If targeting specific contractor, send to their personal room
            if (targetContractor) {
                const contractorRoom = this.roomManager.getContractorRoomName(targetContractor);
                this.io.to(contractorRoom).emit(eventType, {
                    ...taskData,
                    timestamp: new Date().toISOString(),
                });
                logger.info(`Task event ${eventType} sent to contractor ${targetContractor}`);
                return;
            }
            // Broadcast to location-based rooms if location provided
            if (location) {
                const contractorsInRange = this.roomManager.getContractorsInLocation(location.lat, location.lng, location.radius);
                for (const contractorId of contractorsInRange) {
                    if (contractorId !== excludeContractor) {
                        const contractorRoom = this.roomManager.getContractorRoomName(contractorId);
                        this.io.to(contractorRoom).emit(eventType, {
                            ...taskData,
                            timestamp: new Date().toISOString(),
                        });
                ...
                logger.info(`Task event ${eventType} broadcast to ${contractorsInRange.length} contractors in location`);
                return;
            }
            // Broadcast to skill-based rooms if skills provided
            if (skills && skills.length > 0) {
                const contractorsWithSkills = this.roomManager.getContractorsWithSkills(skills);
                ...
                logger.info(`Task event ${eventType} broadcast to ${contractorsWithSkills.length} contractors with matching skills`);
                return;
            }
     ...
     */
    async broadcastSystemNotification(notification, options) {
        try {
            ...
            if (targetContractor) {
                ...
                logger.info(`System notification sent to contractor ${targetContractor}`);
            }
            else if (targetSkills && targetSkills.length > 0) {
                // Send to contractors with specific skills
                const contractorsWithSkills = this.roomManager.getContractorsWithSkills(targetSkills);
                for (const contractorId of contractorsWithSkills) {
                    const contractorRoom = this.roomManager.getContractorRoomName(contractorId);
                    this.io
                        .to(contractorRoom)
                ...
                logger.info(`System notification sent to ${contractorsWithSkills.length} contractors with matching skills`);
            }
            else {
