...
class QuickBooksIntegrationTest {
  ...
  async runTests() {
    try {
      ...
      logger.info(
        "🎉 All QuickBooks integration tests completed successfully!",
      );
    } catch (error) {
  ...
  async testEncryption() {
    ...
    try {
      ...
      logger.info(
        `Generated encryption key: ${encryptionKey.substring(0, 8)}...`,
      );
  ...
  async testContractorCreation() {
    ...
    try {
      ...
      this.testContractors.push(contractor);
  ...
  async testQuickBooksService() {
    ...
    try {
      ...
      const { authUrl, state } = quickbooksService.instance.generateAuthUrl(
      ...
      if (status.connected) {
        throw new Error(
          "Contractor should not be connected to QuickBooks initially",
        );
      }
  ...
  async testTokenManagement() {
    ...
    try {
      ...
      const retrievedToken = await QuickBooksToken.findByContractor(
        contractor._id,
      );
      if (!retrievedToken) {
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  testSuite
    ...
    .then(() => {
      logger.info(
        "🎉 QuickBooks integration test suite completed successfully!",
      );
      process.exit(0);
    })
    .catch((error) => {
...
//# sourceMappingURL=test-quickbooks.js.map
