...
const TaskSchema = new Schema({
    ...
    location: {
        ...
        validate: {
            ...
            message: "Location must be a valid GeoJSON Point with coordinates [longitude, latitude] in valid ranges",
        },
    },
    address: {
        type: String,
...
// Static methods
TaskSchema.statics.findByOrderId = function (orderId) {
...
TaskSchema.statics.findByDateRange = function (startDate, endDate) {
    return this.find({
        scheduledDateTime: {
...
TaskSchema.statics.findAvailableNearLocation = function (lat, lng, radiusInMeters, contractorSkills, excludeContractorId) {
    const query = {
        status: "Pending",
        location: {
            $near: {
...
TaskSchema.statics.findByContractor = function (contractorId, status) {
    const query = { assignedContractors: contractorId };
    if (status) {
        query.status = status;
    }
