...
class RealtimeTestSuite {
  ...
  private async createTestContractorsInDB(): Promise<void> {
    ...
    for (const contractor of this.testContractors) {
      ...
      logger.info(
        `✅ Created contractor in DB: ${contractor.name} (${contractor.id})`,
      );
    }
  ...
  private async connectContractor(contractor: TestContractor): Promise<Socket> {
    return new Promise((resolve, reject) => {
      ...
      socket.on("connect", () => {
        logger.info(
          `✅ Contractor ${contractor.name} connected (${socket.id})`,
        );
      ...
      socket.on("connect_error", (error: any) => {
        logger.error(
          `❌ Connection failed for ${contractor.name}:`,
          error.message,
        );
        reject(error);
      });
  ...
  private async testLocationUpdates(): Promise<void> {
    ...
    logger.info(
      `📍 Sent location update for ${contractor.name}:`,
      testLocation,
    );
  ...
  private async testTaskSubscriptions(): Promise<void> {
    ...
    for (const contractor of this.testContractors) {
      ...
      logger.info(
        `📋 ${contractor.name} subscribed to tasks with filters:`,
        filters,
      );
    }
