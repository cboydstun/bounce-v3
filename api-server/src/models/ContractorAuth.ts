...
export interface IContractorAuthDocument extends Omit<IContractorAuth, "_id">, mongoose.Document {
  comparePassword(candidatePassword: string): Promise<boolean>;
  generateRefreshToken(): string;
  removeRefreshToken(token: string): void;
}

export interface IContractorAuthModel extends mongoose.Model<IContractorAuthDocument> {
...
const ContractorAuthSchema = new Schema<IContractorAuthDocument, IContractorAuthModel>(
  {
    name: {
      type: String,
      required: true,
...
// Instance methods
ContractorAuthSchema.methods.comparePassword = async function (candidatePassword: string): Promise<boolean> {
...
ContractorAuthSchema.methods.removeRefreshToken = function (token: string): void {
...
// Indexes
ContractorAuthSchema.index({ email: 1 });
