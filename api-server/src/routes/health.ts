...
function checkMetricsHealth(): ServiceHealth {
  const startTime = Date.now();
...
async function checkCloudinaryHealth(): Promise<ServiceHealth> {
  const startTime = Date.now();
...
async function checkQuickBooksHealth(): Promise<ServiceHealth> {
  const startTime = Date.now();
