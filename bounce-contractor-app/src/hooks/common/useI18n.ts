...
export const useI18n = (): UseI18nReturn => {
  ...
  const formatDateMemo = useCallback((date: Date) => formatDate(date, locale), [locale]);
  const formatDateTimeMemo = useCallback((date: Date) => formatDateTime(date, locale), [locale]);
  const formatTimeMemo = useCallback((date: Date) => formatTime(date, locale), [locale]);
  const formatTaskTimeMemo = useCallback((date: Date) => formatTaskTime(date, locale), [locale]);
  const formatCurrencyMemo = useCallback((amount: number) => formatCurrency(amount, locale), [locale]);
  const formatDistanceMemo = useCallback((meters: number) => formatDistance(meters, locale), [locale]);
  const formatRelativeTimeMemo = useCallback((date: Date) => formatRelativeTime(date, locale), [locale]);
